    ...
    for i in range(N):
        ...
        for j in range(l - 1, 0, -1):
            ...
            if x[i][j] == pad_token_id and x[i][j - 1] != pad_token_id:
                mask[i, j] = True; found = True; break
        if not found:
            mask[i, 0] = True
    return mask
...
def forward_get_log_probs(sequence_ids, model, attn_mask, inp_len, generated_ids, temperature):
...
def rollout(model_inputs, model, max_new_tokens, temperature, rollout_bs, pad_token_id):
    ...
    all_seq_ids  = torch.full((N, inp_len + max_new_tokens), pad_token_id, dtype=torch.long,  device=device)
    all_gen_ids  = torch.full((N, max_new_tokens),           pad_token_id, dtype=torch.long,  device=device)
    ...
    for i in range(N // rollout_bs):
        ...
        out    = model.generate(
            **batch, return_dict_in_generate=True, output_logits=True,
    ...
    model.train()
    return {"sequence_ids": all_seq_ids, "log_probs_old": all_lp_old, "generated_ids": all_gen_ids}
...
@torch.no_grad()
def rollout_values(sequence_ids, lm_model, value_head, seq_mask, inp_len, gen_len, rollout_bs):
    """
    Returns old_values: (N, gen_len + 1)
    ...
    for i in range(N // rollout_bs):
        ...
        out    = lm_model(batch_seq, attention_mask=batch_mask,
                          position_ids=pos_ids, output_hidden_states=True)
        hidden = out.hidden_states[-1]          # (bs, inp_len+gen_len, H)
        # positions inp_len-1 .. inp_len+gen_len-1 → states s_0 .. s_T
        vals   = value_head(hidden[:, inp_len - 1: inp_len + gen_len])  # (bs, gen_len+1)
        values[s:e] = vals
...
def compute_gae(per_token_rewards, values, gamma, gae_lambda):
    ...
    values            : (N, T+1)  — V(s_0)..V(s_T); last col is bootstrap
    Returns:
...
def forward_ppo(sequence_ids, lm_model, value_head, attn_mask, inp_len, generated_ids, temperature):
    """
    Single forward pass returning:
        log_prob : (N, gen_len)
        values   : (N, gen_len + 1)   — col gen_len is the bootstrap value
    """
    pos_ids = attn_mask.long().cumsum(-1) - 1
    pos_ids.masked_fill_(attn_mask == 0, 1)
    out     = lm_model(sequence_ids, attention_mask=attn_mask,
                       position_ids=pos_ids, output_hidden_states=True)
    logits  = out.logits
    ...
    values     = value_head(hidden[:, inp_len - 1:])                    # (N, gen_len+1)
...
def evaluate(model, tokenizer, val_dataloader, max_new_tokens):
    ...
    for d_idx, (q_lst, a_lst) in enumerate(tqdm(val_dataloader, desc="val", total=len(val_dataloader))):
        inputs  = tokenizer(build_prompt(tokenizer, q_lst),
                            padding=True, padding_side="left", return_tensors="pt").to(device)
        inp_len = inputs["input_ids"].shape[1]
        seq_ids = model.generate(**inputs, do_sample=False, max_new_tokens=max_new_tokens)
        ...
        if os.path.isdir(ckp_dir):
            for name in os.listdir(ckp_dir):
                if name.startswith("step_") and os.path.isdir(os.path.join(ckp_dir, name)):
                    try: candidates.append((int(name.split("_")[1]), name))
...
if __name__ == '__main__':
    ...
    if is_dist:
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=train_prompt_size,
                                      collate_fn=collect_fn, sampler=train_sampler, drop_last=True)
    else:
        train_dataloader = DataLoader(train_dataset, batch_size=train_prompt_size,
                                      collate_fn=collect_fn, shuffle=True, drop_last=True)
    ...
    if is_dist:
        ...
        val_dataloader = DataLoader(val_dataset, batch_size=val_bs,
                                    collate_fn=collect_fn, sampler=val_sampler, drop_last=True)
    else:
        val_dataloader = DataLoader(val_dataset, batch_size=val_bs,
                                    collate_fn=collect_fn, shuffle=False, drop_last=True)
    ...
    for ep in range(start_epoch, num_epoch):
        ...
        for b_idx, (q_lst, a_lst) in enumerate(
            tqdm(train_dataloader, desc=f"ep[{ep}/{num_epoch}]", total=len(train_dataloader))
        ):
            ...
            N, inp_len = model_inputs["attention_mask"].shape
            T = max_new_tokens  # shorthand

            # ── Rollout: generate + old log probs ─────────────────────────────
            rollout_res   = rollout(model_inputs, lora_model, T, temperature, rollout_mini_bs, tokenizer.pad_token_id)
            sequence_ids  = rollout_res["sequence_ids"]    # (N, inp_len+T)
            generated_ids = rollout_res["generated_ids"]   # (N, T)
            log_probs_old = rollout_res["log_probs_old"]   # (N, T)

            seq_mask  = make_loss_mask(sequence_ids, tokenizer.pad_token_id)   # (N, inp_len+T)
            gen_mask  = (generated_ids != tokenizer.pad_token_id).to(log_probs_old.dtype)  # (N, T)
            real_len  = gen_mask.sum(dim=-1)  # (N,)

            # log gen-length stats
            if is_dist:
                rl_g = [torch.zeros_like(real_len) for _ in range(dist.get_world_size())]
                dist.all_gather(rl_g, real_len)
                real_len_all = torch.cat(rl_g)
            else:
                real_len_all = real_len
            if is_main_process:
                logger.add_scalar("rollout/mean_gen_len",     real_len_all.mean().item(),                       g_step)
                logger.add_scalar("rollout/gen_len_clip_frac",(real_len_all == T).float().mean().item(),        g_step)

            generated_text_lst = get_generated_text_lst(generated_ids, tokenizer)

            # ── Compute rewards ───────────────────────────────────────────────
            with torch.no_grad():
                reward_lst_  = [cal_reward(p, gt) for p, gt in zip(generated_text_lst, a_lst_expand)]
                ...
                per_token_rewards = torch.zeros(N, T, device=device)

                if use_ref_model:
                    ref_log_prob = forward_get_log_probs(
                        sequence_ids, ref_model, seq_mask, inp_len, generated_ids, temperature
                    )  # (N, T)
                    per_token_kl = (log_probs_old - ref_log_prob) * gen_mask   # KL ≥ 0 on average
                    per_token_rewards = -kl_weight * per_token_kl

                # Add scalar reward at the last real token of each sample
                last_real_idx = (gen_mask.sum(dim=-1) - 1).long().clamp(min=0)  # (N,)
                per_token_rewards[torch.arange(N, device=device), last_real_idx] += scalar_reward

            # ── Old values (no grad) ──────────────────────────────────────────
            with torch.no_grad():
                old_values = rollout_values(
                    sequence_ids, lora_model, value_head,
                    seq_mask, inp_len, T, rollout_mini_bs,
                )  # (N, T+1)

            # ── GAE ───────────────────────────────────────────────────────────
            with torch.no_grad():
                advantages, returns = compute_gae(per_token_rewards, old_values, gamma, gae_lambda)
                # (N, T), (N, T)
                ...
                if normalize_adv:
                    ...
                    advantages = advantages * gen_mask   # zero-out padding after normalization

                adv_for_train = advantages.unsqueeze(-1)  # (N, T, 1) — broadcast with mask

                # log rollout reward std as proxy for advantage spread
                if is_main_process:
                    logger.add_scalar("rollout/reward_mean", scalar_reward.mean().item(), g_step)
                    logger.add_scalar("rollout/reward_std",  scalar_reward.std(unbiased=False).item(), g_step)
            ...
            for ppo_epoch in range(ppo_num_epoch):
                ...
                for mb_idx in range(num_batches):
                    ...
                    idx = batch_indices[mb_idx * ppo_train_mini_bs: (mb_idx + 1) * ppo_train_mini_bs]

                    mb_seq_ids   = sequence_ids[idx]     # (mb, inp_len+T)
                    mb_gen_ids   = generated_ids[idx]    # (mb, T)
                    mb_seq_mask  = seq_mask[idx]         # (mb, inp_len+T)
                    mb_lp_old    = log_probs_old[idx]    # (mb, T)
                    mb_adv       = adv_for_train[idx]    # (mb, T, 1)
                    mb_returns   = returns[idx]          # (mb, T)
                    mb_val_old   = old_values[idx, :T]   # (mb, T)

                    # token mask for loss computation
                    mask = make_loss_mask(mb_gen_ids, tokenizer.pad_token_id).to(mb_lp_old.dtype)  # (mb, T)

                    # ── Forward ──────────────────────────────────────────────
                    active_lm = ddp_model      if is_dist else lora_model
                    active_vh = ddp_value_head if is_dist else value_head

                    mb_log_prob, mb_values = forward_ppo(
                        mb_seq_ids, active_lm, active_vh,
                        mb_seq_mask, inp_len, mb_gen_ids, temperature,
                    )   # (mb, T), (mb, T+1)
                    mb_new_values = mb_values[:, :T]   # (mb, T)  drop bootstrap

                    # ── Policy loss (clipped surrogate) ───────────────────────
                    log_ratio = (mb_log_prob - mb_lp_old.detach()) * mask
                    ratio     = torch.exp(log_ratio.clamp(-20, 20))
                    pg1       = ratio           * mb_adv.detach().squeeze(-1)
                    pg2       = torch.clamp(ratio, 1 - eps_clip, 1 + eps_clip) * mb_adv.detach().squeeze(-1)
                    pg_loss_matrix = torch.min(pg1, pg2) * mask
                    pg_loss_sample = pg_loss_matrix.sum(dim=-1) / (mask.sum(dim=-1) + 1e-5)
                    pg_loss        = -pg_loss_sample.mean()

                    # ── Value loss (clipped MSE) ──────────────────────────────
                    tgt             = mb_returns.detach()
                    val_unclipped   = (mb_new_values - tgt) ** 2
                    val_clipped     = (mb_val_old + (mb_new_values - mb_val_old).clamp(-value_clip_eps, value_clip_eps) - tgt) ** 2
                    val_loss_matrix = torch.max(val_unclipped, val_clipped) * mask
                    val_loss_sample = val_loss_matrix.sum(dim=-1) / (mask.sum(dim=-1) + 1e-5)
                    ...
                    if entropy_coef > 0:
                        ...
                        entropy_sample = entropy_matrix.sum(dim=-1) / (mask.sum(dim=-1) + 1e-5)
                    ...
                    sync_ctx = (ddp_model.no_sync() if is_dist and is_accumulating else nullcontext())
                    with sync_ctx:
                    ...
                    if (micro_step + 1) % gradient_accumulation_steps == 0:
                        ...
                        grad_norm = torch.nn.utils.clip_grad_norm_(all_params_for_clip, max_grad_norm)
                        if is_main_process:
                            logger.add_scalar("train/grad_norm", grad_norm.item(), g_step)
                            logger.add_scalar("lr", cur_lr, g_step)

                        if not torch.isfinite(grad_norm):
                            if is_main_process:
                                print(f"[warn] non-finite grad_norm at g_step {g_step}, skip")
                        ...
                        with torch.no_grad():
                            # log losses
                            if is_main_process:
                                logger.add_scalar("train/pg_loss",  pg_loss.item(),  g_step)
                                logger.add_scalar("train/val_loss", val_loss.item(), g_step)
                                if entropy_coef > 0:
                                    logger.add_scalar("train/entropy", entropy_loss.item(), g_step)

                            # clip fraction
                            is_clipped  = (pg1 < pg2).to(mask.dtype)
                            clip_mat    = is_clipped * mask
                            if is_dist:
                                cf_g  = [torch.zeros_like(clip_mat) for _ in range(dist.get_world_size())]
                                gm_g  = [torch.zeros_like(mask)     for _ in range(dist.get_world_size())]
                                dist.all_gather(cf_g,  clip_mat)
                                dist.all_gather(gm_g,  mask)
                                clip_frac = torch.cat(cf_g).sum() / (torch.cat(gm_g).sum() + 1e-5)
                            else:
                                clip_frac = clip_mat.sum() / (mask.sum() + 1e-5)
                            if is_main_process:
                                logger.add_scalar("train/clip_frac", clip_frac.item(), g_step)

                            # entropy (for logging even when coef=0)
                            ent_mat    = -mb_log_prob.detach() * mask
                            ent_sample = ent_mat.sum(dim=-1) / (mask.sum(dim=-1) + 1e-5)
                            if is_dist:
                                eg = [torch.zeros_like(ent_sample) for _ in range(dist.get_world_size())]
                                dist.all_gather(eg, ent_sample)
                                ent_sample = torch.cat(eg)
                            if is_main_process:
                                logger.add_scalar("train/entropy", ent_sample.mean().item(), g_step)
            ...
            if (b_idx + 1) in eval_steps:
                ...
                if is_main_process:
                    ...
                    print(f"eval ep{ep} step{b_idx+1} ({frac:.0%}): f_r={val_f_r:.4f}  c_r={val_c_r:.4f}")
                    logger.add_scalar("val/f_r", val_f_r, global_step=g_step)
                    logger.add_scalar("val/c_r", val_c_r, global_step=g_step)
